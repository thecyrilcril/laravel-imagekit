...
- **Cleanup: delete the Source once ImageKit holds the file.** A new `cleanup` boolean on a profile (default `false`, so upgrading deletes nothing) and a fluent `->cleanup()` / `->cleanup(false)` on the media-library chain, next to `->await()`, drop the original, its conversions and its responsive images from the media disk once the row carries `imagekit.file_id`. It runs as a queued job, `Thecyrilcril\ImageKit\Jobs\CleanupSource`, dispatched after commit from the two success sites (the awaited upload and the queued `PushFileToImageKit`), on `queue.names.cleanup` with the shared connection, tries and backoff. The job re-checks the row and deletes nothing when the row is gone or has no `file_id`; missing files are not an error; it removes files through media-library's own remover and throws `Thecyrilcril\ImageKit\Exceptions\CleanupFailed` when the original, a registered conversion or a recorded responsive image is still on disk afterwards, so the queue retries. It logs one warning, after the delete succeeds, on a row that carries responsive-image data, so a retry does not repeat it. `->cleanup()` on a collection without `->toImageKit()` throws `UnregisteredCollection`; the override never stays in `custom_properties`. `ImageKit::cleanup($modelClass, $collection)` queues one job per row with a `file_id`, skips rows pending deletion, and returns the count. `ImageKit::fake()` queues the job on a faked awaited upload. Why a queued job: [ADR-0003](docs/adr/0003-cleanup-is-a-deferred-job-that-removes-the-whole-source.md) ([#26](https://github.com/thecyrilcril/laravel-imagekit/issues/26)).
...
- **Profile and no-delete assertions on `ImageKit::fake()`.** `assertUploaded($media, profile: 'photos')` passes only when a recorded upload for that row used that profile; without a profile it behaves as before. A collection registered with a plain `->toImageKit()` records the profile as `'default'`, so `profile: 'default'` matches it. `assertNothingDeleted()` passes when nothing was deleted and lists the deleted ids when it fails. The fake's bulk `cleanup(string $modelClass, string $collection): int` returns 0, mirroring `backfill()` ([#25](https://github.com/thecyrilcril/laravel-imagekit/issues/25)).

### Changed

- **Contract change for custom `ImageKitClient` implementations.** `upload()` and `uploadNow()` take a third parameter, `?bool $cleanup = null` (null means "use the Profile"), and the contract gains `cleanup(string $modelClass, string $collection): int`. `PushFileToImageKit` takes the same `?bool $cleanup = null` after `profile`. The two success sites and the fake now share one routine, `Thecyrilcril\ImageKit\Support\MarkUploaded`, that saves the row, fires `FileUploaded` and queues Cleanup in that order, and resolves the Cleanup decision (the per-call override, else the Profile's `cleanup` flag) in one place ([#26](https://github.com/thecyrilcril/laravel-imagekit/issues/26)).
- **`ImageKit::fake()` now persists a faked awaited upload.** On success, `uploadNow()` writes `imagekit.file_id` (`fake-{id}`) and `imagekit.file_path` (`/` + the folder the package resolves + `/` + the file name) on the row, saves it, and fires `FileUploaded` with the result it returns. The row is ready after a faked `await: true` upload, `->await()` or `uploadNow()`, and `getUrl()` returns the ImageKit URL, as in production. A test that read `custom_properties` after a faked awaited upload and expected it empty must expect the two keys now. `failUploads()` still writes nothing, and a queued `upload()` still writes nothing. Because the fake now reads the profile's `cleanup` flag, `uploadNow()` on the fake throws `UnknownProfile` for a profile name that is not configured, as the real manager does ([#25](https://github.com/thecyrilcril/laravel-imagekit/issues/25)).
