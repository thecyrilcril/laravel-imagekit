...
- **Cleanup: delete the Source once ImageKit holds the file.** A new `cleanup` boolean on a profile (default `false`, so upgrading deletes nothing) and a fluent `->cleanup()` / `->cleanup(false)` on the media-library chain, next to `->await()`, drop the original, its conversions and its responsive images from the media disk once the row carries `imagekit.file_id`. It runs as a queued job, `Thecyrilcril\ImageKit\Jobs\CleanupSource`, dispatched after commit from the two success sites (the awaited upload and the queued `PushFileToImageKit`), on `queue.names.cleanup` with the shared connection, tries and backoff. The job re-checks the row and deletes nothing when the row is gone or has no `file_id`; missing files are not an error; it removes files through media-library's own remover and throws `Thecyrilcril\ImageKit\Exceptions\CleanupFailed` when the original is still on disk afterwards, so the queue retries. It logs one warning on a row that carries responsive-image data. `->cleanup()` on a collection without `->toImageKit()` throws `UnregisteredCollection`; the override never stays in `custom_properties`. `ImageKit::cleanup($modelClass, $collection)` queues one job per row with a `file_id`, skips rows pending deletion, and returns the count. `ImageKit::fake()` queues the job on a faked awaited upload. Why a queued job: [ADR-0003](docs/adr/0003-cleanup-is-a-deferred-job-that-removes-the-whole-source.md) ([#26](https://github.com/thecyrilcril/laravel-imagekit/issues/26)).
