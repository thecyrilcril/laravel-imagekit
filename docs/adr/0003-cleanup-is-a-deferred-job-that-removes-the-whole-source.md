...
- `ImageKitClient::uploadNow()` and `ImageKitClient::upload()` gain a third optional parameter for the per-call override, because the queued path also goes through the bound client so that `ImageKit::fake()` intercepts it, and the override must reach the job as a constructor argument. The contract also gains the bulk `cleanup()`. Custom implementations of the contract must add all three.
